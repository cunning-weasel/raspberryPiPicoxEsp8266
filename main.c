...
int main() {
    ...
    uart_write(command_CWMODE, strlen(command_CWMODE));
    uart_write("\r\n", 2);
    delay_ms(1000);

    // set SoftAP name
    char command_CWSAP[] = "AT+CWSAP=" pos_softap "," ",11,0,3";
    uart_write(command_CWSAP, strlen(command_CWSAP));
    uart_write("\r\n", 2);
    delay_ms(1000);
    char res[20];
    ...
    uart_write(command_CIPMUX, strlen(command_CIPMUX));
    uart_write("\r\n", 2);
    delay_ms(1000);
    ...
    uart_write(command_CIPSERVER, strlen(command_CIPSERVER));
    uart_write("\r\n", 2);
    delay_ms(2000);
    ...
    while (1) {
        ...
        uart_write(command_CIPSEND, strlen(command_CIPSEND));
        uart_write("\r\n", 2);
        delay_ms(2000);
